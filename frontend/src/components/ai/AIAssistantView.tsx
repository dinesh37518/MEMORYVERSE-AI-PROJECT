...
export const AIAssistantView: React.FC = () => {
  const { user, documents, skills, projects, internships, certifications, setPreviewDoc } = useApp();

  const [apiKey, setApiKey] = useState(() => getStoredApiKey());
  const [showApiKeyInput, setShowApiKeyInput] = useState(false);
  const [showKeyText, setShowKeyText] = useState(false);

  // Custom Groq Prompt State
  ...
  const handleSaveCustomPrompt = () => {
    ...
    setPromptSavedNotice('Custom Groq AI Prompt saved successfully! Active for all AI responses.');
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      const response = await generateGroqResponse(query, contextData, apiKey);
      ...
      if (response.isRealAi) {
        setActiveModel(response.modelUsed || 'Groq LLaMA-3.3 70B');
      } else {
        setActiveModel(null);
      }
    } catch (err) {
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] px-2.5 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300 font-bold border border-indigo-500/30 flex items-center gap-1">
                <Sparkles className="w-3 h-3 text-indigo-400" /> Powered by Groq LLaMA-3.3
              </span>
            </div>
        ...
        <div className="p-4 bg-purple-950/90 border-b border-purple-500/40 space-y-3 animate-in slide-in-from-top duration-300">
          <div className="flex items-center justify-between">
            <h4 className="text-xs font-bold text-purple-200 flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-purple-400" /> Groq Agent Custom Prompt Editor
            </h4>
          ...
          <p className="text-xs text-purple-300">
            Paste or customize your prompt instructions below. When saved, Groq AI will process all responses using your prompt guidelines:
          </p>
          ...
          <textarea
            ...
            placeholder="Paste your custom Groq prompt instructions here..."
        ...
        <div className="p-4 bg-indigo-950/90 border-b border-indigo-500/30 flex flex-wrap items-center justify-between gap-3 animate-in slide-in-from-top duration-300">
          <div className="flex-1 min-w-[280px]">
            ...
            <p className="text-[10px] text-slate-400 mt-1">
              Key is stored securely in your local browser storage and used directly for Groq LLaMA-3.3 API requests.
            </p>
          </div>
        ...
        {isTyping && (
          <div className="flex items-center gap-3">
            ...
            <div className="soft-3d-card p-3.5 rounded-2xl text-xs text-slate-400 animate-pulse flex items-center gap-2">
              ...
              {apiKey ? 'Generating response via Groq AI (LLaMA-3.3)...' : 'Searching MemoryVerse RAG knowledge vault...'}
            </div>
          </div>
        )}
